...
# print(df.tail())
# print(df.shape)
lossdf = pd.read_csv("./dataset/loss_rate.csv")
lossdf = lossdf.sort_values('name')
lossdf.columns =['idx', "Sta", "loss"]
...
pygmt.makecpt(cmap="jet", series=[0, 1])
fig.plot(
    x=df.iloc[:,2], # lon
    y=df.iloc[:,1], # lat
    style="c0.15c",
...
fig.colorbar(frame=['x0.2'])
fig.savefig("figs/greenland_loss.png")